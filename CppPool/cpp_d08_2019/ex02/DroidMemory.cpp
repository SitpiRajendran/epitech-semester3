...
bool DroidMemory::operator==(DroidMemory &right) const
{
    if (_exp == right.getExp() && _finger == right.getExp())
        return true;
    else
        return false;
}
...
bool DroidMemory::operator>=(size_t right) const
{
    if (_exp >= right)
        return true;
    else
